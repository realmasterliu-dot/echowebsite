...
> 状态：工程骨架已搭建（uni-app Vue3）；业务内容填充进行中
...
源站：`jiangcheng-{300,400,500,600,700}.woff2`（五档，体积可观）。

小程序建议：

- 首期仅嵌入 **400 + 500**（或仅用系统字体 + 主色强调）
- 若用自定义字体：放 `static/fonts/`，用 `@font-face`，并评估主包大小
- 备选：标题用图片（Logo/slogan 图），正文用系统字体（PingFang / 微软雅黑）
...
| Grainient WebGL                                            | 用静态渐变图或 CSS 渐变背景替代    |
| favicon / icons.svg（Web）                                 | 小程序用各自 tab 图标              |

### 9.4 包体积控制清单

- [ ] 图片压缩（pngquant / tinypng）
- [ ] 大图考虑 CDN（若已有域名与合法业务域名配置）
- [ ] 图标单个 &lt; 5KB 量级
- [ ] 主包 &lt; 2MB；超出则分包（about 页可作分包候选）
...
- [ ] 使用官方模板创建 uni-app Vue3 Vite 工程于 `E:\echowebsite-wechat`
- [ ] 配置微信小程序 `appid`（测试号可先）
- [ ] 建立本文目录骨架（空页面可跑通）
- [ ] 接入微信开发者工具预览
...
- [ ] 拆分迁移 `content.js` → `data/*`
- [ ] 迁移 `variables` → SCSS
- [ ] 迁入 logo / mockup 等图片；确定字体策略
- [ ] 配置 `pages.json` 四 Tab
- [ ] 实现 `PageShell`、`SectionHeader`
- [ ] 实现 `services/phone.js`、`navigation.js`

### 阶段 2：核心页面填充（2～3 天）

- [ ] 首页：Hero（静态背景）+ Company + Business + CTA
- [ ] 价格页：四套餐卡片
- [ ] 联系页：流程 + 拨号 + 结语
- [ ] 真机点通：Tab 切换、拨打电话
...
- [ ] 拆四个 Why 区块组件
- [ ] 统计数字：先静态，再视需要加 CountUp
- [ ] 增长图：CSS/静态条形，不引入 GSAP
...
- [ ] 无「页面内写死大段业务文案」（应来自 data）
- [ ] 无单文件超过约定体量而不拆分
- [ ] `data` 不引用 UI；组件不依赖具体页面路径硬编码（除 navigation 封装）
- [ ] 主包体积符合微信限制

### 14.3 体验验收

- [ ] 首屏 2 秒内有品牌识别（Logo + 名称）
- [ ] 无明显布局错乱、文字溢出
- [ ] 底部安全区不被 Tab 遮挡内容
...
- `echowebsite-wechat` 独立初始化仓库（当前为空目录）
