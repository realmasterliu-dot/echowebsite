...
| 静态图 | `static/images/`：logo.png / logo.jpg / logo-horizontal.png / mockup-intro.jpg |
...
| 构建产物 | `dist/dev/mp-weixin`（开发中）；`dist/build/mp-weixin` ≈ **925KB / 172 文件**（&lt; 2MB 主包上限） |
...
| `static/icons/content` 切图 | ✅ | FA → PNG，`npm run icons:export` |
...
| 品牌字体（mp） | ✅ 方案 C | `loadFontFace` + base64 Data URL（`npm run fonts:encode`）；无需 CDN |
...
// data/services.js
{
  icon: '/static/icons/service-local-life.png', // 或 iconKey + 映射表
  title: '本地生活整合运营',
  desc: '...',
}
```
...
export const iconMap = {
  'object-ungroup': '/static/icons/object-ungroup.png',
  // ...
}
```
...
| 源路径                         | 目标             | 用途         |
| ------------------------------ | ---------------- | ------------ |
| `public/logo.png` / `logo.jpg` | `static/images/` | 品牌         |
| `public/logo-horizontal.png`   | `static/images/` | 横版 Logo    |
| `public/mockup-intro.jpg`      | `static/images/` | 公司介绍配图 |
| 品牌主色相关导出图（若有）     | `static/images/` | 首屏背景替代 |

### 9.2 字体策略

源站：`jiangcheng-{300,400,500,600,700}.woff2`（五档，合计约 690KB）。

**已定案（已落地）：**

- 嵌入 **300 / 400 / 500 / 600 / 700** 五档（约 690KB woff2）→ `src/static/fonts/`
- H5：`@font-face` → `fonts.scss`（`#ifdef H5`）
- **微信小程序（方案 C）**：`npm run fonts:encode` → `jiangcheng-base64.js`；`App.onLaunch` → `uni.loadFontFace` Data URL（**无需 CDN**）；`mp-weixin.libVersion` ≥ 3.7.9
- Token：`$fw-light` 300 · `$fw-regular` 400 · `$fw-medium` 500 · `$fw-semibold` 600 · `$fw-bold` 700
- 开关：`config.js` → `SWITCH.USE_BRAND_FONT`
...
- [~] 主包 &lt; 2MB — **当前 `dist/build/mp-weixin` ≈ 925KB，未超限；正式上传前再核一次**
...
- [x] 迁移 `variables` → SCSS
- [x] 迁入 logo / mockup 等图片；确定字体策略 — **图片齐全；字体仅嵌 400+500（mp 回退系统字）**
...
- [ ] 图片压缩与主包体积正式签收 — **构建约 925KB，压缩清单未勾完**
- [ ] iOS / Android 真机各测一遍
...
- [~] 主包体积符合微信限制 — **构建约 925KB &lt; 2MB；压缩优化仍可做**
