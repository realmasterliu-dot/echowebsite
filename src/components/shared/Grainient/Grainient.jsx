...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    // WebGL may be unavailable (disabled, blocklisted, context limit, headless).
    // If context creation fails we MUST NOT throw — otherwise the error bubbles
    // up with no boundary and unmounts the whole React tree (blank white page).
    ...
    // If the GL context is later lost, drop the canvas gracefully instead of
    // throwing inside the render loop.
    const onContextLost = (e) => { e.preventDefault(); tryStop(); };
